...
/*
A bunch of queries of the DVDStore database
*/
...
FROM   INFORMATION_SCHEMA.COLUMNS AS isc
       INNER JOIN INFORMATION_SCHEMA.TABLES AS ist ON isc.table_name = ist.table_name
...
SELECT s.staff_id
     , SUM(p.amount) AS 'August_2005_amount'
FROM   staff AS s
...
SELECT film.title
     , COUNT(film.title) AS 'Number_of_Rentals'
FROM   rental
...
SELECT store.store_id
     , FORMAT(SUM(amount), 'C', 'en-US') AS 'Revenue'
FROM   store
